...
function App() {
  ...
  const minterContract = new provider.eth.Contract(MetaAds.abi, MetaAdsAddress.MetaAds);
      ...
      try {
       ...
       const chainId = await provider.eth.net.getId();
        ...
        if (!supportedNetworks[chainId]){
          alert("Please change your network to Evmos Testnet")
        }
  ...
  const addNFT = async ( price, dailyVisits, name, description, ipfsurl, address )=>{
     ...
     //const ethAddress = await addressConverter.evmosToEth(address);
...
const buyySpace = async (
  ...
  price,
      ...
      try {
        const _price = ethers.utils.parseUnits(String(price), "ether");
        await minterContract.methods.buySpace(index, days, expiryDate, headline, visualLink, adDescription).send({ from: address, value: _price });
      } catch (error) {
        console.log({ error });
      }
  } 
